...
class TestSoul(unittest.TestCase):
    ...
    def testCheckNamesWithSpacesParsing(self):
        ...
        gate = tale.base.Exit(room, "the gate", direction="gate")
        door1 = tale.base.Exit(room, "door number one", direction="door one")
        door2 = tale.base.Exit(room, "door number two", direction="door two")
    ...
    def testEnterExits(self):
        ...
        player = tale.player.Player("julie", "f")
        room = tale.base.Location("somewhere")
        gate = tale.base.Exit(room, "gate", direction="gate")
        east = tale.base.Exit(room, "east", direction="east")
        door1 = tale.base.Exit(room, "door number one", direction="door one")
    ...
    def testParse(self):
        ...
        player = tale.player.Player("julie", "f", "human")
        room = tale.base.Location("somewhere")
        south_exit = tale.base.Exit(room, "a door to the south", direction="south")
        east_exit = tale.base.Exit(room, "a door to the east", direction="east")
