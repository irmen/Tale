...
def print_object_location(player, obj, container, print_parentheses=True):
    ...
    if container in player:
        if print_parentheses:
            ...
            player.tell("%s was found in %s, in your inventory." % (languagetools.capital(obj.name), container.title))
    elif container is player.location:
        if print_parentheses:
            player.tell("(%s was found in your current location)" % obj.name)
        else:
            player.tell("%s was found in your current location." % languagetools.capital(obj.name))
    elif container is player:
        if print_parentheses:
            player.tell("(%s was found in your inventory)" % obj.name)
        else:
            player.tell("%s was found in your inventory." % languagetools.capital(obj.name))
    else:
        if print_parentheses:
            player.tell("(%s was found in %s)" % (obj.name, container.name))
        else:
            player.tell("%s was found in %s." % (languagetools.capital(obj.name), container.name))
